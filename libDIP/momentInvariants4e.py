...
def compute_eta(m):
    ...
    # eta30
    eta['eta30'] = (m['m30'] - 3 * xbar * m['m20'] + 2 * (xbar**2) * m['m10']) / (m00**2.5)
    
    # eta03
    eta['eta03'] = (m['m03'] - 3 * ybar * m['m02'] + 2 * (ybar**2) * m['m01']) / (m00**2.5)
    
    # eta21
    # MATLAB: (m.m21 - 2 * xbar * m.m11 - ybar * m.m20 + 2 * xbar^2 * m.m01)
    eta['eta21'] = (m['m21'] - 2 * xbar * m['m11'] - ybar * m['m20'] + 2 * (xbar**2) * m['m01']) / (m00**2.5)
    
    # eta12
    # MATLAB: (m.m12 - 2 * ybar * m.m11 - xbar * m.m02 + 2 * ybar^2 * m.m10)
    eta['eta12'] = (m['m12'] - 2 * ybar * m['m11'] - xbar * m['m02'] + 2 * (ybar**2) * m['m10']) / (m00**2.5)
...
def compute_phi(e):
    ...
    phi[4] = (e['eta30'] - 3 * e['eta12']) * t1 * (t1**2 - 3 * t2**2) + \
             (3 * e['eta21'] - e['eta03']) * t2 * (3 * t1**2 - t2**2)
             
    # phi6
    # (eta20 - eta02) * [ t1^2 - t2^2 ] + 4*eta11 * t1 * t2
    phi[5] = (e['eta20'] - e['eta02']) * (t1**2 - t2**2) + \
             4 * e['eta11'] * t1 * t2
    ...
    phi[6] = (3 * e['eta21'] - e['eta03']) * t1 * (t1**2 - 3 * t2**2) + \
             (3 * e['eta12'] - e['eta30']) * t2 * (3 * t1**2 - t2**2) # Note 3*eta12 - eta30 is Negative of phi5's 1st term's (eta30 - 3*eta12)
