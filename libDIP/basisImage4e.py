...
def basisImage4e(xform: Any = "DCT", N: Any = 4, P: Any = 2, display: Any = True):
    """
    ...
    for i in range(N):
        for j in range(N):
            ...
            if xform == "DFT":
                ...
                term = np.outer(
                    A[i, :], A[j, :]
                )  # This creates complex matrix if A is complex
                S = np.abs(term)
            elif xform == "DFTr":
    ...
    for i in range(N):
        for j in range(N):
            ...
            S_DISPLAY[xd : xd + block_size, yd : yd + block_size] = S_LINE[
                xs : xs + block_size, ys : ys + block_size
            ]
