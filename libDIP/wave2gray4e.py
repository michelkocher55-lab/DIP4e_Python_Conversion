...
def wave2gray4e(c: Any, s: Any, scale: Any = 1, border: Any = "absorb"):
    """
    ...
    for i in range(N_levels):
        ...
        # Padding function
        def pad_coeff(arr: Any, p_total: Any, val: Any):
            """pad_coeff."""
            if np.all(p_total == 0):
                return arr
            ...
            return np.pad(
                arr,
                ((p_pre[0], p_post[0]), (p_pre[1], p_post[1])),
                mode="constant",
                constant_values=val,
            )
...
def mat2gray(img: Any, limits: Any = None):
    """
