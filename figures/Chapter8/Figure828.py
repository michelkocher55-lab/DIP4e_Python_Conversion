...
def blkidct(x, a, qmatrix):
    x = qmatrix * x
    return a @ x @ a.conj().T


def threshold_coding(x, qmatrix):
...
def process(f, t, block_size, qmatrix):
    y = blockproc_square(f, block_size, lambda b: blkdct(b, t), pad_partial_blocks=True)
    y = blockproc_square(y, block_size, lambda b: threshold_coding(b, qmatrix), pad_partial_blocks=True)
    y = blockproc_square(y, block_size, lambda b: blkidct(b, t.conj().T, qmatrix), pad_partial_blocks=True)
    return np.real(y)
