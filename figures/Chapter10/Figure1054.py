...
def slic_compat(image, n_segments):
    try:
        return slic(image, n_segments=n_segments, compactness=10, start_label=1, channel_axis=-1 if image.ndim == 3 else None)
    except TypeError:
        return slic(image, n_segments=n_segments, compactness=10, start_label=1, multichannel=(image.ndim == 3))
...
if BW.mean() > 0.95:
    print('Fallback: boundary mask is too dense; using SLIC superpixels for stable regions.')
