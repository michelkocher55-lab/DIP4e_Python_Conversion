...
for iter_idx in range(3):
    ...
    plt.imshow(AddSE2Image(CO[:, :, iter_idx], ia.iasedisk(Rad[iter_idx]), mx), cmap='gray')
...
for iter_idx in range(3):
    ...
    plt.imshow(AddSE2Image(ASF[:, :, iter_idx], ia.iasedisk(Rad[iter_idx]), mx), cmap='gray')
...
plt.bar(centers1, counts1, width=(centers1[1] - centers1[0]) if centers1.size > 1 else 1.0)
