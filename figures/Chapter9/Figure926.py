...
for k in range(levels):
    ...
    if k == 0:
        ...
        Skeleton[:, :, k] = ia.iaaddm(Skeleton[:, :, k - 1], IndividualSkeleton[:, :, k])

    DilIndividualSkeleton[:, :, k] = ia.iadil(IndividualSkeleton[:, :, k], ia.iasebox(k))

    if k == 0:
        ReconstructedImage[:, :, k] = DilIndividualSkeleton[:, :, k]
    else:
        ReconstructedImage[:, :, k] = ia.iaaddm(ReconstructedImage[:, :, k - 1], DilIndividualSkeleton[:, :, k])
