...
class SimpleMLP:
    def __init__(self, input_size, hidden_size, output_size, learning_rate=0.02, seed=None):
...
def Figure1335UsingNNToolbox():
    ...
    plt.plot(best_epoch, best_val, 'go', markersize=10, fillstyle='none', markeredgewidth=2, label='Best')
    ...
    plt.stem(Target[0, :], linefmt='b-', markerfmt='bo', label='Out1', use_line_collection=True)
    ...
    x = np.arange(4)
    plt.stem(x - 0.1, Target[0, :], linefmt='b-', markerfmt='bo', label='Class 1', use_line_collection=True)
    plt.stem(x + 0.1, Target[1, :], linefmt='r-', markerfmt='rs', label='Class 2', use_line_collection=True)
    plt.legend()
    plt.title('target')
    
    plt.subplot(2, 2, 4)
    plt.stem(x - 0.1, OutputRaw[0, :], linefmt='b-', markerfmt='bo', label='Class 1', use_line_collection=True)
    plt.stem(x + 0.1, OutputRaw[1, :], linefmt='r-', markerfmt='rs', label='Class 2', use_line_collection=True)
    plt.title('output')
