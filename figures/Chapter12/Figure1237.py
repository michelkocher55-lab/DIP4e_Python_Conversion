...
fR45 = rotate(Motif.astype(float), Theta1, reshape=True, order=1, mode='constant', cval=0.0, prefilter=False)
fR45 = np.clip(np.rint(fR45), 0, 255).astype(np.uint8)

fR90 = rotate(Motif.astype(float), Theta2, reshape=True, order=1, mode='constant', cval=0.0, prefilter=False)
...
ax2.set_xticklabels(['None', 'Translation', 'Scaling', 'Mirror', 'Rotation45', 'Rotation90'])
