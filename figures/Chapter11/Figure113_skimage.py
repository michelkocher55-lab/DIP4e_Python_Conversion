...
# Add libDIPUM to path if not present (to find snakeMap4e)
edge_lib_path = '/Users/michelkocher/Desktop/DIP4e_Python_Conversion/libDIPUM'
...
def Figure113_skimage():
    ...
    def get_var(name):
        if name not in mat_data: return None
        val = mat_data[name]
    ...
    if xi.ndim > 1: xi = xi.flatten()
    if yi.ndim > 1: yi = yi.flatten()
    ...
    def run_snake_on_map(emap, label=""):
        ...
        try:
            ...
            snake = active_contour(emap, init_snake, alpha=Alpha, beta=Beta, 
                                   w_line=1, w_edge=0, gamma=0.01, max_iterations=2500,
    ...
    def plot_snake(ax, img_bg, snake, emap_disp, title):
    ...
    ax_m[0,0].imshow(emap1, cmap='gray'); ax_m[0,0].set_title("Energy Map: Both")
    ax_m[0,1].imshow(emap2, cmap='gray'); ax_m[0,1].set_title("Energy Map: After")
    ax_m[1,0].imshow(emap3, cmap='gray'); ax_m[1,0].set_title("Energy Map: Before")
    ax_m[1,1].imshow(emap4, cmap='gray'); ax_m[1,1].set_title("Energy Map: None")
    plt.tight_layout()
