...
def edgemodel(edge_type: Any, M: Any, N: Any, ILOW: Any, IHIGH: Any, WIDTH: Any):
    """
    ...
    midpoint = int(
        np.floor(N / 2)
    )  # Python 0-based index of the start of the right side?
    ...
    if edge_type == "step":
        ...
        if WIDTH == 1:
            ...
            count = 0
            for k in range(
                firstpoint + 1, lastpoint
            ):  # range excludes end, so lastpoint is excluded
                count += 1
