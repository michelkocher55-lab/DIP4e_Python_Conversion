...
def wavefast(x, n, *args):
    ...
    x = np.asarray(x)
    sx = x.shape
    if x.ndim not in (2, 3) or min(sx[:2]) < 2 or not np.isrealobj(x) or not np.issubdtype(x.dtype, np.number):
        raise ValueError("X must be a real, numeric 2-D or 3-D matrix.")

    if (lp.ndim != 1 or hp.ndim != 1 or not np.isrealobj(lp) or not np.isrealobj(hp)
            or not np.issubdtype(lp.dtype, np.number)
            or not np.issubdtype(hp.dtype, np.number)
            or fl != len(hp) or (fl % 2) != 0):
        raise ValueError("LP and HP must be even and equal length real, numeric filter vectors.")
...
def double_page(x, i, pages):
    if pages == 1:
        return x.astype(float)
    return x[:, :, i].astype(float)


def _addcoefs(c, x, pages):
...
def _extend(x, fl, pages, extmode):
...
def _convolve(x, h, axis_type, fl, keep, pages, extmode):
