...
def cellplot(c, lims=None):
    ...
    def _draw_cell(obj, x0, y0, w, h, depth=0):
        if isinstance(obj, (list, tuple)):
            ...
            pad = 0.04 * min(w, h)
            rect = Rectangle((x0, y0 - h), w, h, facecolor='none', edgecolor='k', linewidth=1.0)
            ax.add_patch(rect)
            ...
            rect = Rectangle((x0 + pad_leaf, y0 - h + pad_leaf),
                             w - 2 * pad_leaf, h - 2 * pad_leaf,
                             facecolor=color, edgecolor='k', linewidth=0.5)
            ax.add_patch(rect)
            handles.append(rect)
            # Add text for scalar numbers/short strings
            if isinstance(obj, (int, float, np.number)):
                ax.text(x0 + w / 2, y0 - h / 2, str(int(obj)) if float(obj).is_integer() else str(obj),
                        ha='center', va='center', fontsize=9, color='k')
            elif isinstance(obj, str) and len(obj) <= 10:
                ax.text(x0 + w / 2, y0 - h / 2, obj,
                        ha='center', va='center', fontsize=9, color='k')
    ...
    if legend:
        cb = plt.colorbar(plt.cm.ScalarMappable(cmap=cmap), ax=ax, ticks=np.linspace(0.1, 0.9, 5))
