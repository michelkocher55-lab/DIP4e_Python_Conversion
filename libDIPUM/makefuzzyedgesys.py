...
def makefuzzyedgesys(save_path='fuzzyedgesys.pkl'):
    """
    ...
    def zero(z): return bellmf(z, -0.3, 0)
    
    # not_used = @(z) onemf(z); -> Always 1?
    # I don't have onemf. I'll implement a lambda.
    def not_used(z): return np.ones_like(z, dtype=float)
    
    # Output MFs
    # black = @(z) triangmf(z,0,0,0.75);
    def black(z): return triangmf(z, 0, 0, 0.75)
    
    # white = @(z) triangmf(z,0.25,1,1);
    def white(z): return triangmf(z, 0.25, 1, 1)
