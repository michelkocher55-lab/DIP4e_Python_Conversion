...
class FuzzyApproximator:
    def __init__(self, interpolator, grid, num_inputs):
...
def approxfcn(F, range_val):
    ...
    table_dim = min(int(np.floor(max_table_elements**(1/num_inputs))), max_table_dim)
    ...
    # Create interpolator
    interp = RegularGridInterpolator(tuple(grids), table, bounds_error=False, fill_value=None)
