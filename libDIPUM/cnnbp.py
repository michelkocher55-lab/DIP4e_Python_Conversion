...
def cnnbp(net: Any, y: Any):
    ...
    n = len(
        net["layers"]
    )  # Number of layers (including input which is 0-indexed in Python?)
        ...
        if layer["type"] == "c":
            ...
            for i in range(len(layer["a"])):
                ...
                for j in range(len(next_layer["a"])):
                    ...
                    res = signal.convolve(
                        next_d, rot_kernel[..., np.newaxis], mode="full"
                    )
                    z = z + res
    ...
    for l in range(1, num_layers):
        ...
        if layer["type"] == "c":
            ...
            layer["dk"] = [
                [None for _ in range(num_outputs)] for _ in range(num_inputs)
            ]
            ...
            for j in range(num_outputs):
                for i in range(num_inputs):
                    ...
                    dk_res = signal.convolve(
                        prev_a_flipped, layer["d"][j], mode="valid"
                    )
                    # Reshape to 2D
