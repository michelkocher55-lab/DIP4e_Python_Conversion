...
def cellcomplex(I, cellsize):
    ...
    Ssel = sparse.coo_matrix(
        (np.full(len(rr), cellsize, dtype=int), (rr, cc)),
        shape=I_padded.shape
    ).tocsr()
...
def mppvertices(R, cellsize):
    ...
    if len(rows) == 0: return np.array([]), np.array([])
    ...
    if len(L) == 0: return np.array([]), np.array([])
...
def trace_boundary(bin_img, start_rc):
    """
