...
from helpers.libdipum.huffman import huffman
...
def mat2huff(x: Any):
    ...
    Returns dict with fields:
      code : uint16 row vector of packed 16-bit words
      min  : uint16 (xmin biased by +32768)
      size : uint32 size of input matrix
      hist : uint16 histogram in unit-width bins [xmin, xmax]
    """
    x = np.asarray(x)
    if x.ndim == 1:
        x = x.reshape(1, -1)
