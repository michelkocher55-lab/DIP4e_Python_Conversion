...
def gaussian_2d(X: Any, mean: Any, cov: Any):
    """gaussian_2d."""
...
def doog1(sig: Any, r: Any, th_deg: Any, N: Any):
    """doog1."""
    no_pts = N
...
def doog2(sig: Any, r: Any, th_deg: Any, N: Any):
    """doog2."""
    no_pts = N
    rng = np.arange(-(N / 2) + 0.5, (N / 2) + 0.5)
    x, y = np.meshgrid(rng, rng)
...
def make_filterbank_odd2(num_ori: Any, filter_scales: Any, wsz: Any, enlong: Any = 3):
    """make_filterbank_odd2."""
    enlong = enlong * 2
...
def make_filterbank_even2(num_ori: Any, filter_scales: Any, wsz: Any, enlong: Any = 3):
    """make_filterbank_even2."""
    enlong = enlong * 2
    num_scale = len(filter_scales)
    ori_incr = 180.0 / num_ori
    ori_offset = ori_incr / 2.0
...
def quadedgep(I: Any, par: Any = None, threshold: Any = 0.2):
    """quadedgep."""
    I = I.astype(float)
    r, c = I.shape
    if par is None:
    ...
    gx_1 = gx[Y, X]
    gy_1 = gy[Y, X]
    Y_n = Y + h_vals.astype(int)
    X_n = X + v_vals.astype(int)
    Y_n = np.clip(Y_n, 0, r - 1)
    X_n = np.clip(X_n, 0, c - 1)
    gx_2 = gx[Y_n, X_n]
    gy_2 = gy[Y_n, X_n]
    gx_out = gx_1 + gx_2
    gy_out = gy_1 + gy_2
...
def computeEdges(imageX: Any, parametres: Any = None, threshold: Any = 0.02):
    """computeEdges."""
    I = imageX
...
def computeW(
    imageX: Any, dataW: Any = None, emag: Any = None, ephase: Any = None, **kwargs: Any
):
    """computeW."""
    if dataW is None:
        pass
    ...
    for dy in range(-r, r + 1):
        for dx in range(-r, r + 1):
            ...
            dst_indices = indices[
                y_start + dy : y_end + dy, x_start + dx : x_end + dx
            ].flatten()
            ...
            if sample_rate < 1.0:
                ...
                sel_rows = (
                    src_indices // nc
                )  # Wait, src_indices is index in flattened array
            ...
            lx_prev = line_x[0]
            mag_prev = emag[
                y_start + ly_prev : y_end + ly_prev, x_start + lx_prev : x_end + lx_prev
            ]
            phase_prev = ephase[
                y_start + ly_prev : y_end + ly_prev, x_start + lx_prev : x_end + lx_prev
            ]
            ...
            for k in range(1, len(line_y)):
                ...
                # Slices for k
                mag_curr = emag[
                    y_start + ly_curr : y_end + ly_curr,
                    x_start + lx_curr : x_end + lx_curr,
                ]
                phase_curr = ephase[
                    y_start + ly_curr : y_end + ly_curr,
                    x_start + lx_curr : x_end + lx_curr,
...
def ncut(W: Any, nbEigenValues: Any = 8):
    """ncut."""
    N = W.shape[0]
...
def discretisation(EigenVectors: Any):
    ...
    try:
        ...
        try:
            ...
            import helpers.libgeneral.kmeans
...
def NcutImage(I: Any, nbSegments: Any = 10, **kwargs: Any):
    """NcutImage."""
