...
def levelSetIterate4e(phi, F, delT=None):
    """
    ...
    # Upwind Norm Grad
    # gNormPlus
    gNormPlus = np.sqrt(np.maximum(Dxminus, 0)**2 + np.minimum(Dxplus, 0)**2 +
                        np.maximum(Dyminus, 0)**2 + np.minimum(Dyplus, 0)**2)
                        
    # gNormMinus
    gNormMinus = np.sqrt(np.maximum(Dxplus, 0)**2 + np.minimum(Dxminus, 0)**2 +
                         np.maximum(Dyplus, 0)**2 + np.minimum(Dyminus, 0)**2)
