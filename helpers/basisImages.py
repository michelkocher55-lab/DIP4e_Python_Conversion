...
def basisImages(A: Any, gray: Any = 0.5, space: Any = 1):
    """
    ...
    if M < 2 or M != N or space < 0 or gray < 0 or gray > 1:
        raise ValueError(
            "A must be square with at least 2 rows, GRAY must be between 0 and 1, and SPACE must be non-negative!"
        )
    ...
    for part in parts:
        ...
        for i in range(N):
            for j in range(N):
                ...
                display_img[
                    r_start + 1 : r_start + 1 + N, c_start + 1 : c_start + 1 + N
                ] = img_chunk
