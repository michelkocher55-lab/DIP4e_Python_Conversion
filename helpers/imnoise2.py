...
def imnoise2(f: Any, type_noise: Any, a: Any = None, b: Any = None):
    """
    ...
    if f.dtype.kind != "f":
        ...
        if f.max() > 1.0:
            f_float = (
                f.astype(float) / 255.0
            )  # Naive scaling? Or assume user handles it?
...
def saltpepper(M: Any, N: Any, a: Any, b: Any):
    """saltpepper."""
    # Check probabilities
    if (a + b) > 1:
