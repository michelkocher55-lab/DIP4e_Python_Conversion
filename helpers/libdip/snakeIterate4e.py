...
def snakeIterate4e(alpha, beta, gamma, x, y, NI, Fx, Fy):
    """
    ...
    b4 = -4 * np.ones(K - 1)
    c4 = np.ones(K - 2)
    D4 = np.diag(a4) + np.diag(b4, -1) + np.diag(b4, 1) + np.diag(c4, -2) + np.diag(c4, 2)
    ...
    interp_fx = RegularGridInterpolator((r_grid, c_grid), Fx, bounds_error=False, fill_value=0)
    interp_fy = RegularGridInterpolator((r_grid, c_grid), Fy, bounds_error=False, fill_value=0)
