...
def blockTransformTest4e(f, type_transform, blockSize, truncate):
    """
    ...
    if pad_r > 0 or pad_c > 0:
        f_padded = np.pad(f, ((0, pad_r), (0, pad_c)), mode='constant', constant_values=0)
    else:
        f_padded = f
    ...
    reconstructed_padded = np.zeros_like(f_padded, dtype=complex) # Transforms might be complex
    ...
    for i in range(n_br):
        for j in range(n_bc):
            r_start = i * blockSize
            c_start = j * blockSize
            reconstructed_padded[r_start:r_start+blockSize, c_start:c_start+blockSize] = rec_blocks_4d[i, j]
