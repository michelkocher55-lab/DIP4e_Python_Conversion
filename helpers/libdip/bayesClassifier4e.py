import numpy as np
from helpers.libdip.mahalanobisDistance4e import mahalanobisDistance4e
...
def bayesClassifier4e(X, CA, MA, P=None):
    ...
    if CA.shape[1] == n and CA.shape[2] == n:
        ...
        raise ValueError(f"CA shape {CA.shape} does not match expected dimensions for n={n}.")
    ...
    if MA.shape[0] != nc:
         raise ValueError(f"Number of mean vectors {MA.shape[0]} does not match number of covariance matrices {nc}.")
    ...
    if P is None:
        ...
        if len(P) != nc:
             raise ValueError(f"Number of priors {len(P)} does not match number of classes {nc}.")
