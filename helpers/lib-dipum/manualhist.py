...
try:
    from twomodegauss import twomodegauss
except ImportError:
    try:
        from helpers.twomodegauss import twomodegauss
    except ImportError:
        # Fallback if in same dir but script run directly
        pass
...
def manualhist(*args: Any):
    ...
    # Import check inside function to robustly handle import issues if run as script
    if "twomodegauss" not in globals():
        import sys
        import os
        ...
        try:
            ...
            print("Error: twomodegauss not found.")
            return None
