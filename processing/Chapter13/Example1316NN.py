...
def main(show: bool = True) -> None:
    """Run Example1316NN.py with explicit input/output paths."""
    input_data_dir = "AllDataFiles"
    output_dir = str(PROJECT_ROOT / "output")
    output_figure_base = str(Path(output_dir) / "Example1316NN")
    os.makedirs(output_dir, exist_ok=True)

    result = Dip().example1316nn(data_dir=input_data_dir)
