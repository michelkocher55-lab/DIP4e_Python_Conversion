...
def main(show: bool = True) -> None:
    """Run Example1315CNN.py with explicit input/output paths."""
    input_data_dir = "AllDataFiles"
    output_dir = str(PROJECT_ROOT / "output")
    output_figure_base = str(Path(output_dir) / "Example1315CNN")
    os.makedirs(output_dir, exist_ok=True)

    result = Dip().example1315cnn(data_dir=input_data_dir)
