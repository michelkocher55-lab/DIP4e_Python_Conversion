...
def main(show: bool = True) -> None:
    """Run FigureTifs2cv.py with explicit input/output paths."""
    input_data_dir = "AllDataFiles"
    output_dir = str(PROJECT_ROOT / "output")
    output_figure_base = str(Path(output_dir) / "FigureTifs2cv")
    os.makedirs(output_dir, exist_ok=True)

    result = Dip().figuretifs2cv(data_dir=input_data_dir)
